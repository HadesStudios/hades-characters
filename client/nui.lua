...
RegisterNUICallback('GetData', function(data, cb)
    ...
    Callbacks:ServerCallback('Characters:GetServerData', {}, function(serverData)
        ...
        print('[CLIENT] Requesting characters from server...')
        Callbacks:ServerCallback('Characters:GetCharacters', {}, function(characters)
            print('[CLIENT] Received characters from server:', characters and #characters or 'nil')
            print('[CLIENT] Setting player coords and camera...')
            ...
            DestroyCam(cam)
            cam = cam2
            print('[CLIENT] Camera setup complete')

            for k, v in ipairs(characters) do
                print('[CLIENT] Processing character', k, ':', v.First, v.Last, 'HasPreview:', v.Preview and true or false)
                if v.Preview then
                    print('[CLIENT] Loading preview model:', v.Preview.model)
                    loadModel(joaat(v.Preview.model))
                    print('[CLIENT] Creating preview ped...')
                    local ped = CreatePed(
                        5,
                    ...
                    print('[CLIENT] Waiting for ped to exist...')
                    while not DoesEntityExist(ped) do
                        Wait(1)
                    end

                    print('[CLIENT] Ped exists, setting coords and freezing...')
                    SetEntityCoords(ped, previews[k][1], previews[k][2], previews[k][3], 0.0, 0.0, 0.0, false)
                    FreezeEntityPosition(ped, true)
                    print('[CLIENT] Applying preview customization...')
                    Ped:Preview(ped, tonumber(v.Gender), v.Preview, false, v.GangChain)
                    print('[CLIENT] Preview applied for character', k)
                    print('[CLIENT] Preview applied for character', k)

                    table.insert(peds, ped)
                else
                    print('[CLIENT] No preview, creating default ped...')
                    loadModel(tonumber(v.Gender) == 0 and `mp_m_freemode_01` or `mp_f_freemode_01`)
                    print('[CLIENT] Default model loaded')
                    local ped = CreatePed(
                        5,
...
RegisterNUICallback('PlayCharacter', function(data, cb)
    ...
    Callbacks:ServerCallback('Characters:GetCharacterData', data.character.ID, function(cData)
        ...
        TriggerEvent('Characters:Client:SetData', -1, cData, function()
            exports['hades-base']:FetchComponent('Spawn'):SpawnToWorld(cData, function()
