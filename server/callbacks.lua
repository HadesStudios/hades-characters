...
function RegisterCallbacks()
	...
	Callbacks:RegisterServerCallback('Characters:GetCharacters', function(source, data, cb)
		...
		Logger:Trace('Characters', string.format('Getting characters for player %s', source))
		Database.Game:find({
			...
			if not success then
				Logger:Error('Characters', string.format('Failed to get characters for player %s', source))
				cb(nil)
				return
			end
			Logger:Trace('Characters', string.format('Found %s characters for player %s', #results, source))
			local cData = {}
			for k, v in ipairs(results) do
				Logger:Trace('Characters', string.format('Processing character %s (%s %s) for player %s', v._id, v.First, v.Last, source))
				local p = promise.new()
				...
				Database.Game:findOne({
					...
					Logger:Trace('Characters', string.format('Ped query callback fired for character %s, success: %s', v._id, tostring(s2)))
					local preview = false
					if pedData and pedData[1] and pedData[1].Ped then
						preview = pedData[1].Ped
						Logger:Trace('Characters', string.format('Found ped preview for character %s', v._id))
					else
						Logger:Trace('Characters', string.format('No ped preview for character %s', v._id))
					end
				...
				Logger:Trace('Characters', string.format('Awaiting ped query for character %s...', v._id))
				Citizen.Await(p)
				Logger:Trace('Characters', string.format('Ped query completed for character %s', v._id))
			end
			player:SetData('Characters', cData)
			Logger:Trace('Characters', string.format('Sending %s characters to player %s', #cData, source))
			cb(cData)
		end)
	end)
