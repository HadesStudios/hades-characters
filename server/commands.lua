function RegisterCommands()
	Chat:RegisterStaffCommand('logout', function(source, args, rawCommand)
		exports['hades-base']:FetchComponent('Execute'):Client(source, 'Characters', 'Logout')
	end, {
		help = 'Logout',
	}, 0)
			...
			if char ~= nil then
				...
				if phonePermissions[app] then
					if phonePermissions[app][perm] ~= nil then
						...
						Chat.Send.System:Single(source, 'Permission Doesnt Exist')
					end
				else
					Chat.Send.System:Single(source, 'App Doesnt Exist')
				end
			end
		else
			Chat.Send.System:Single(source, 'Invalid Target')
